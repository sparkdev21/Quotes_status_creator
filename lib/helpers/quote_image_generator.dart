...
class QuoteImageGenerator {
  ...
  Future<Image> _drawFooter(Image quoteImage) async {
    ...
    final logoHeightCenterPosition =
    ...
    var result = copyInto(quoteImage, logoBackground,
        dstX: logoPosition['x']!.toInt() - backgroundMargin,
        dstY: logoPosition['y']!.toInt());

    return copyInto(result, qbLogo,
        dstX: logoPosition['x']!.toInt(), dstY: logoPosition['y']!.toInt());
  }
  ...
  Future<List<int>> generateImage() async {
    ...
    if (quoteImage.height > availableHeightForQuote) {
      quoteImage =
          _resizeImageByHeight(quoteImage, availableHeightForQuote.toInt());
    }

    if (quoteImage.width > availableWidthForQuote) {
      print("Image resized because too big on width");
      quoteImage =
          _resizeImageByWidth(quoteImage, availableWidthForQuote.toInt());
    }
